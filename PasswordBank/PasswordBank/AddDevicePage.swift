...
class AddDevicePage: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        mainController.devicePasswordAdded = self.devicePasswordAdded
    ...
    @IBAction func CreateNewPassword(_ sender: UIButton) {
        ...
        {
            userDevicePasswords[username]![passName] = encrypt(newPass, decryption1(ciphers[username]!))
            devicePasswordAdded = true
            WarningLabel.isHidden = true
            performSegue(withIdentifier: "NewDeviceToMain", sender: self)
        }
        else
        {
